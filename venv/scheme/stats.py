...
from scheme.resynthesis.resynthesis_external_stats import external_reliability
...
def k_exp(sch, capacity=32, debug=False):
    n = sch.inputs()
    m = sch.elements()

    capacity = min(capacity, 2 ** sch.inputs())

    def count_errors(output_dicts):
        errors_number = 0
        for label in sch.element_labels():
            output_true_error = ((output_dict[None], output_dict[label]) for output_dict in output_dicts if label in output_dict)
            errors = (i ^ j for i, j in output_true_error)
    ...
    for inps in inputs_combinations(n, capacity):
        input_dicts = [{None: inp} for inp in inps]
        start = time.time()
        output_dicts = sch.process_dicts(input_dicts, capacity)
        if debug:
            print(time.time() - start)
        start = time.time()
        errors += count_errors(output_dicts)
        if debug:
            print(time.time() - start)
...
def form_pyramid(inp_arr, out, l=0, res_arr = []):
    if len(inp_arr) == 2:
        line = inp_arr[0] + " " + inp_arr[1] + " " + out
        print(line)
        res_arr.append(line)
        return res_arr
    k = 0
    ...
    for inp in inp_arr:
        ...
        if k == 2:
            k = 0
            l += 1
            line = line + out + "_" + str(l)
            print(line)
            res_arr.append(line)
            inp_arr2.append(out + "_" + str(l))
            line = ""
    if k==1:
        inp_arr2.append(inp_arr[-1])
    return (form_pyramid(inp_arr2, out, l, res_arr))
...
if __name__ == '__main__':
    ...
    def plot_polynomial(polynomial):
        t = [Fraction(i) for i in np.linspace(0, 1, 100)]
        plt.plot(t, np.polyval(list(reversed(polynomial)), t))

    c17 = sa.read_scheme('c17.txt')
    min_polynomial, max_polynomial = eof_p_interval(c17, max_tests=100)
    plot_polynomial(min_polynomial)
    plot_polynomial(max_polynomial)
    plt.show()


def correlation(scheme, tests=0):
    random.seed()
    err = []
    correlatio = []
    ...
    for i in range(tests):
        rnd_inputs = random.randint(0, 2 ** scheme.inputs())
        rnd_elem = random.randint(0, scheme.elements()-1)
        for i in range(scheme.elements()):
            err += [0]
        err[rnd_elem] = 1
        input_values = num2vec(rnd_inputs, scheme.inputs())
        znch_schm = scheme.process(num2vec(rnd_inputs, scheme.inputs()))
        znch_schm_err = scheme.process(input_values, err)
        match = 0
        err = []
        for i in range(scheme.outputs()):
            if znch_schm[i] == znch_schm_err[i]:
                 match += 1
        correlatio[scheme.outputs() - match] += 1
    return correlatio
...
def correlatio_lvl(scheme, tests):
    level = scheme.label_levels()
    keys = dict.keys(level)
    values = dict.values(level)
    mapkeys = []
    for key in keys:
        mapkeys += [key]
    i = 0
    ...
    err = []
    correlatio = []
    elements = sorted(scheme.__elements__.keys())
    elem_numb = 0
    arr_of_elem_numb = []
    for element in elements:
        arr_of_elem_numb += [level[element]]
        elem_numb += 1
    uniqarr = uniq(arr_of_elem_numb)
    correlatio_lvl = array([])
    correlatio_lvl.resize((len(uniqarr)+1, scheme.outputs()+1))
    for i in range(scheme.outputs()+1):
        correlatio += [0]
    for i in range(tests):
        rnd_inputs = random.randint(0, 2 ** scheme.inputs())
        ...
        err[rnd_elem] = 1
        input_values = num2vec(rnd_inputs, scheme.inputs())
        znch_scheme = scheme.process(num2vec(rnd_inputs, scheme.inputs()))
        znch_scheme_err = scheme.process(input_values, err)
        match = 0
        err = []
        for i in range(scheme.outputs()):
            if znch_scheme[i] == znch_scheme_err[i]:
                 match += 1
        correlatio_lvl[arr_of_elem_numb[rnd_elem]][scheme.outputs() - match] += 1
        correlatio[scheme.outputs() - match] += 1
    res = []
    for i in range(len(uniqarr)):
        res += [Expected_value(correlatio_lvl[i+1])]
    # res.append(';')
    # res.extend(correlatio)
    return res
