...
def print_circuit_in_verilog_file(circ, circname, file_name):
    ...
    for el in circ.__elements__:
        ...
        inps = circ.__elements__[el][1]
        #new
        if circ.__elements__[el][0] == 'GND':
            str_3 = '\t' + 'assign ' + el + ' = 0;'+ '\n'
        if circ.__elements__[el][0] == 'VCC':
            str_3 = '\t' + 'assign ' + el + ' = 1;'+ '\n'
        if circ.__elements__[el][0] != 'GND' and circ.__elements__[el][0] != 'VCC':
            str_3 = '\t' + type + ' ' + 'e' + elindex.__str__() + ' ('
            str_3 += out
            for inp in inps:
                str_3 += ', ' + inp
            str_3 += ');\n'
...
def print_run_file_opt(run_file, verilog_file, synth_file, graph):
    f = open(run_file, 'w')
    f.write('read_verilog ' + verilog_file + '\n')
    f.write('hierarchy -check -top circ\n')
    f.write('proc; opt; fsm; opt; techmap; opt\n')
    f.write('dfflibmap -liberty std.lib\n')
    f.write('abc -liberty std.lib; opt\n')
    f.write('clean\n')
    f.write('write_verilog ' + synth_file + '\n')
    # f.write('show -format svg -prefix ' + graph + '\n')
    f.close()
...
def find_name_in_elements(nodes, name):
    if name in nodes:
        return 1
    return 0
...
def convert_file_to_relic_format(circuit, synth_file, converted_circuit_file):
    ...
    content = f1.read()
    # Все элементы
    content=content.replace('NXOR','XNOR')
    matches = re.findall(r"\s*?(INV|BUF|AND|NAND|OR|NOR|XNOR|XOR) (.*?) \((.*?);", content, re.DOTALL) #проблема
    # Все assign
    matches2 = re.findall(r"\s*?assign (.*?) = (.*?);", content, re.DOTALL)
    total = len(matches) + len(matches2)

    nodelist = circuit.__inputs__ + circuit.__outputs__
    ...
    for m in matches:
        ...
        if (cell_type != 'INV' and cell_type != 'BUF' and cell_type != 'buf'):
    ...
    for m in matches2:
        if find_name_in_elements(nodelist, m[1]) == 1 \
                and find_name_in_elements(nodelist, m[0]) == 1:
            f.write('BUF ' + m[1] + ' ' + m[0] + '\n')
            #new
        if find_name_in_elements(nodelist, m[0]) == 1 \
                and m[1] == '1\'b0':
            f.write('GND ' + m[0] + '\n')
        if find_name_in_elements(nodelist, m[0]) == 1 \
                and m[1] == '1\'b1':
